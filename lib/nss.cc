...
void aes128gcm(uint8_t *ciphertext, uint8_t *mac, uint8_t *plaintext, int len,
               ...
               const uint8_t *key) {
  PK11SlotInfo *slot = PK11_GetInternalSlot();
  SECItem keyItem = {siBuffer, const_cast<unsigned char *>(key), 16};

  PK11SymKey *symKey =
      PK11_ImportSymKey(slot, CKM_AES_GCM, PK11_OriginUnwrap,
                        CKA_ENCRYPT, &keyItem, nullptr);
  if (!symKey) {
    PK11_FreeSlot(slot);
    return;
  ...
  uint32_t outputLen;
  SECStatus rv =
      PK11_Encrypt(symKey, CKM_AES_GCM, &params, &ciphertext[0],
                   &outputLen, len + 16, plaintext, len);
  if (rv != SECSuccess) {
    PK11_FreeSymKey(symKey);
    PK11_FreeSlot(slot);
